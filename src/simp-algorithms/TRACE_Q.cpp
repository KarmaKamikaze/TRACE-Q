...
namespace trace_q {
    ...
    data_structures::Trajectory TRACE_Q::simplify(data_structures::Trajectory const& original_trajectory, double max_locations, double min_query_accuracy) const {
        ...
        auto points_on_axis = static_cast<int>(std::ceil(1 / grid_density_multiplier));
        ...
        for (int x_point = 0; x_point <= points_on_axis; ++x_point) {
            ...
            auto x_coord = mbr.x_low + x_point * grid_density_multiplier * (mbr.x_high - mbr.x_low);
            for (int y_point = 0; y_point <= points_on_axis; ++y_point) {
                auto y_coord = mbr.y_low + y_point * grid_density_multiplier * (mbr.y_high - mbr.y_low);
        ...
        std::cout << "Query amount: " << query_amount << "\n";
        std::cout << "Query objects length: " << query_objects.size() << "\n";
    ...
    int TRACE_Q::calculate_query_amount() const {
        auto points_in_net = static_cast<int>(std::pow(std::ceil(1.0 / grid_density_multiplier) + 1, 2));
    ...
    std::vector<std::shared_ptr<spatial_queries::Range_Query>> TRACE_Q::range_query_initialization(
        ...
        for (int window_number = 0; window_number < windows_per_grid_point; ++window_number) {
            auto w_x_low = x - 0.5 * (pow(window_expansion_rate, window_number)
                                            * grid_density_multiplier * (mbr.x_high - mbr.x_low));
            ...
            auto w_y_low = y - 0.5 * (pow(window_expansion_rate, window_number)
                                            * grid_density_multiplier * (mbr.y_high - mbr.y_low));
