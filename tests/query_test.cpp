...
#include "../src/querying/Range_Query_Test.hpp"
...
TEST_CASE("Range_Query - operator()") {
    ...
    SUBCASE("operator() is true when original and simplified pass the query") {
        auto query = spatial_queries::Range_Query_Test(trajectories.small, 4.0, 30.0 , 2.0, 15.0, 0.0, 20.0);
    ...
    SUBCASE("operator() is true when original and simplified both fail the query") {
        auto query = spatial_queries::Range_Query_Test(trajectories.small, 4.0, 30.0 , 2.0, 15.0, 20, 25);
        auto in_window = query(simplified_small_trajectory);
    ...
    SUBCASE("operator() is false when original passes while simplified fails the query") {
        auto query = spatial_queries::Range_Query_Test(trajectories.small, 8, 30.0 , 2.0, 15.0, 0, 20);
        auto in_window = query(simplified_small_trajectory);
